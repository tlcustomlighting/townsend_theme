    ...
    <table class="form-table meta-box">
		...
		<tr>
			...
			<td>
				...
				<br />
				<p>
					<?php _e( "Minimum and recommended image dimensions: 220 x 190.", 'onioneye' ); ?>
				</p>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<br />
				<p>
					<?php _e( 'Image dimensions best at 700 x unlimited.', 'onioneye' ); ?>
				</p>
			</td>
		</tr>
    ...
    <table class="form-table meta-box">
		<tr>
			...
			<td><textarea id="portfolio-video-embed" cols="60" rows="3" name="portfolio-video-embed"><?php echo $video_embed_code; ?></textarea>
				...
				<p>
				<?php _e( 'Add the video embed code, generated on the site the video is hosted on, such as youtube and vimeo, to have the video displayed on your project page. Width is best at 700px with any height.', 'onioneye' ); ?>
				</p>
			</td>
		</tr>
	</table>
...
function eq_additional_info_portfolio_metabox() {
	
	global $post;
	
	$client = get_post_meta( $post->ID, 'oy-client', true );
	$project_url = get_post_meta( $post->ID, 'oy-item-url', true );
    ...
    <table class="form-table meta-box">
		...
		<tr>
			<th><label for="oy-client"><?php _e( "Client:", 'onioneye' ); ?></label></th>
			<td>
				<input id="oy-client" class="upload" type="text" name="oy-client" value="<?php echo $client; ?>" />
				<br />
				<p>
					<?php _e( "The name of the client for this portfolio item.", 'onioneye' ); ?>
				</p>
			</td>
		</tr>
		
		<tr>
			<th><label for="oy-item-url"><?php _e( "Item URL:", 'onioneye' ); ?></label></th>
			<td>
				<input id="oy-item-url" class="upload" type="text" name="oy-item-url" value="<?php echo $project_url; ?>" />
				<br />
				<p>
					<?php _e( 'The link/URL of this portfolio item.', 'onioneye' ); ?>
				</p>
			</td>
		</tr>
...
function save_portfolio_meta_box_values( $post_id ) {
    ...
    if( $post->post_type == "portfolio" ) {
        ...
        if ( isset( $_POST['oy-client']) ) { update_post_meta( $post->ID, 'oy-client', stripslashes( htmlspecialchars( $_POST['oy-client'] ) ) ); }
		if ( isset( $_POST['oy-item-url']) ) { update_post_meta( $post->ID, 'oy-item-url', stripslashes( htmlspecialchars( esc_url( $_POST['oy-item-url'] ) ) ) ); }
    }	
}
...
function add_portfolio_meta_boxes() {
	...
	add_meta_box( 'portfolio-additional-info-metabox', __( "Additional Information", 'onioneye' ), 'eq_additional_info_portfolio_metabox', 'portfolio', 'normal', 'high' );
}
